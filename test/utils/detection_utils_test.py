...
class DetectionUtilsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.row_detections = [(1, 1, 2, 3, 4, 2, 4, 0, 0.9), (2, 5, 6, 7, 8, 6, 8, 0, 0.8)]
    ...
    @patch('builtins.open', new_callable=mock_open,
           read_data="frame_index,tracker_id,x1,y1,x2,y2,bdcx,bdcy,class_id,confidence\n5,19,6.0,7.0,8.0,9.0,3.0,2.0,0.0,0.9\n")
    def test_read_predictions_dict(self, mock_file):
        expected_output = {5: [(19, 6.0, 7.0, 8.0, 9.0, 3.0, 2.0, 0, 0.9)]}
        result = DetectionUtils.read_predictions_dict('dummy_path')
        self.assertEqual(result, expected_output)

    @patch('builtins.open', new_callable=mock_open)
    def test_write_predictions_dict(self, mock_file):
        predictions_dict = {5: [(19, 6.0, 7.0, 8.0, 9.0, 3.0, 2.0, 0, 0.9)]}
        DetectionUtils.write_predictions_dict(predictions_dict, 'dummy_path')
        mock_file.assert_called_once_with('dummy_path', 'w')
        mock_file().write.assert_any_call("frame_index,tracker_id,x1,y1,x2,y2,bdcx,bdcy,class_id,confidence\n")
        mock_file().write.assert_any_call("5,19,6.0,7.0,8.0,9.0,3.0,2.0,0,0.9\n")
